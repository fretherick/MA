...
from sigsom.utils.metrics_pytorch import som_loss, kmeans_loss, quantization_error, topographic_error 
...
import sys
# ch score adjusted for the mahalanobis distance
sys.path.append(os.path.abspath('../utils'))
from calinski_harabasz_score_mahalanobis import calinski_harabasz_score_mahalanobis
...
class SOMLayer(nn.Module):
    ...
    def forward(self, inputs, cov_matrix_inv = None):
        ...
        mahalanobis_term = torch.matmul(diff, cov_matrix_inv)
...
class SOMM(nn.Module):
    ...
    def initialize(self,data):
        ...
        self.add_module('som_layer', self.som_layer)  # Register SOMLayer as a module
        print("updated")

    def forward(self, x, cov=None):
        som_output = self.som_layer(x, cov)
        return som_output
    ...
    def fit(self, X_train, 
            ...
            eval_interval=50,
            save_epochs=50, 
            batch_size= 512,# 512,# 512, # 16 instead of 64, 256
            Tmax=10, 
            Tmin=0.1, 
            decay='exponential'):
...
class GSOMM(nn.Module):
    def __init__(self, logger, params, min_improvement=0.1, max_depth=5):
        super(GSOMM, self).__init__()
        self.logger = logger
        self.params = params
        self.min_improvement = min_improvement  # Minimum improvement in quantization error to continue
        self.max_depth = max_depth  # Maximum depth of the hierarchical SOM
        self.somm = SOMM(logger, params)  # Initial SOMM
        self.dbi_threshold = 300
        self.Tmax = params.Tmax
        self.Tmin = params.Tmin
        self.iterations = params.iterations
        self.ch_scores = []
        self.prototypes = {}  # Dictionary to store prototypes hierarchically
        self.covariances = {}  # Dictionary to store covariance matrices at each depth
        self.linkage_matrix = []
        self.cluster_count = 0
        self.split_linkage_matrix = []

    def initial_train(self, X_train):
        self.logger.info(f"Training Initial SOMM")
        self.somm.fit(X_train, X_val=None, iterations=self.iterations, Tmax=self.Tmax, Tmin=self.Tmin)
        
        # Store the initial prototypes at the root level (depth 0)
        # self.prototypes[0] = self.somm.som_layer.prototypes.detach().cpu().numpy()
        epsilon = 1e-6  # Regularization term to prevent singular covariance matrix
        cov_matrix = np.cov(X_train.T) + np.eye(X_train.shape[1]) * epsilon
        cov_matrix_inv = np.linalg.inv(cov_matrix)
        
        # Store the inverse covariance matrix in self.covariances for depth 0
        # self.covariances[0] = cov_matrix_inv
        self.prototypes[0] = {}
        self.prototypes[0][(0,)] = self.somm.som_layer.prototypes.detach().cpu().numpy()
        self.covariances[0] = {}
        self.covariances[0][(0,)] = cov_matrix_inv

        # Get the initial quantization error
        self.eval()
        with torch.no_grad():
            som_output = self.somm.forward(X_train)
            qe = quantization_error(som_output)
        # Predict cluster assignments using the base SOMM
        y_pred = self.somm.predict(X_train)

        return y_pred, qe
    
    def fit(self, X_train, y_pred, prototypes_parent, depth=1, parent_cluster_id=None):
        """
        Fit the hierarchical SOM and store the prototypes and covariances in a proper tree structure.
        
        Args:
            X_train (torch.Tensor): Training data for the current level.
            y_pred (torch.Tensor): Predicted labels (clusters) for the current level.
            prototypes_parent (torch.Tensor): Parent prototypes.
            depth (int): Current depth in the hierarchy.
            parent_cluster_id (tuple or None): Full lineage of parent cluster IDs for this depth.
        """
        unique_clusters = y_pred.unique()
        sub_clusters = []

        # Loop over each unique cluster at the current depth
        for idx, cluster_id in enumerate(unique_clusters):
            print(f"Checking cluster ID {cluster_id} at depth {depth}")
            cluster_data = X_train[y_pred == cluster_id]

            # Create a new SOMM for the sub-cluster
            sub_somm = SOMM(self.logger, self.params)
            sub_somm.initialize(cluster_data)

            # Use parent-level prototypes to compute covariance matrix for quantization error
            parent_prototypes = prototypes_parent[idx].detach()
            print(parent_prototypes)

            # Calculate the covariance matrix using the parent prototypes
            epsilon = 1e-6  # Regularization term to avoid singular covariance matrix
            cov_matrix = np.cov(cluster_data.T)
            cov_matrix += np.eye(cov_matrix.shape[0]) * epsilon
            cov_matrix_inv = torch.tensor(np.linalg.inv(cov_matrix), dtype=torch.float32)

            # Calculate the quantization error for the sub-cluster
            input_expanded = cluster_data.unsqueeze(1)  # Shape: (batch_size, 1, input_dim)
            parent_prototypes_expanded = parent_prototypes.unsqueeze(0)  # Shape: (1, n_parent_prototypes, input_dim)
            diff = input_expanded - parent_prototypes_expanded
            mahalanobis_term = torch.matmul(diff, cov_matrix_inv)
            distances = torch.sum(mahalanobis_term * diff, dim=2)

            initial_error = quantization_error(distances)

            # Fit SOMM to the sub-cluster data
            sub_somm.fit(cluster_data, iterations=self.iterations, Tmax=self.Tmax, Tmin=self.Tmin)
            sub_error = quantization_error(sub_somm.forward(cluster_data))

            self.logger.info(f"Initial Quantization Error: {initial_error}, Sub-SOMM Error: {sub_error}")

            # Calculate Davies-Bouldin Index for the sub-cluster
            sub_cluster_pred = sub_somm.predict(cluster_data)
            unique_sub_clusters = torch.unique(sub_cluster_pred)
            cluster_size = cluster_data.shape[0]

            if len(unique_sub_clusters) > 1:
                ch_score = calinski_harabasz_score_mahalanobis(cluster_data.cpu().numpy(), sub_cluster_pred.cpu().numpy())
                print(f"Calinski Harabasz Index for cluster {cluster_id} at depth {depth}: {ch_score}")
                self.ch_scores.append((cluster_id, depth, ch_score, initial_error - sub_error))

                # Store linkage information
                child_clusters = unique_sub_clusters.tolist()
                self.split_linkage_matrix.append([cluster_id.item(), child_clusters[0], child_clusters[1], cluster_size, depth])
                for _ in unique_sub_clusters:
                    self.linkage_matrix.append([self.cluster_count, cluster_id, depth, cluster_size])
                    self.cluster_count += 1
            else:
                print(f"Skipping Calinski Harabasz Index calculation for cluster {cluster_id} at depth {depth} due to insufficient unique labels.")
                ch_score = -1  # or any placeholder value
                self.ch_scores.append((cluster_id, depth, ch_score, initial_error - sub_error))

            # Construct the full lineage of the parent cluster
            if parent_cluster_id is None:
                full_parent_id = (cluster_id.item(),)  # Start with the current cluster ID if no parent
            else:
                full_parent_id = parent_cluster_id + (cluster_id.item(),)  # Append the current cluster ID to the parent's lineage

            # Use a unique identifier combining depth and the full lineage of parent cluster IDs
            unique_id = (depth,) + full_parent_id  # This ensures uniqueness for each node

            # Check if quantization error improves significantly and CH score is above the threshold
            if (initial_error - sub_error > 310 and depth < self.max_depth and ch_score > 80):#self.min_improvement initial_error - sub_error 
                print(f"Quantization error improvement: {initial_error - sub_error}")
                print(f"Recursing into cluster {cluster_id} at depth {depth + 1}")
                self.logger.info(f"Recursing into cluster {cluster_id} at depth {depth + 1}")

                # Only add prototypes and covariance matrix if we actually make an improvement (split)
                if depth not in self.prototypes:
                    self.prototypes[depth] = {}
                self.prototypes[depth][unique_id] = sub_somm.som_layer.prototypes.detach().cpu().numpy()

                if depth not in self.covariances:
                    self.covariances[depth] = {}
                self.covariances[depth][unique_id] = cov_matrix_inv  # Store the inverse covariance matrix

                # Recursively fit sub-clusters, pass current cluster_id as parent_cluster_id
                sub_cluster_results = self.fit(
                    cluster_data, sub_cluster_pred, sub_somm.som_layer.prototypes, depth=depth + 1, parent_cluster_id=full_parent_id
                )
                sub_clusters.append((cluster_id, cluster_data, sub_cluster_results, sub_somm.som_layer.prototypes.detach().cpu().numpy(), ch_score))
            else:
                print(f"No significant improvement, max depth reached, or CH score exceeded for cluster {cluster_id}.")
                self.logger.info(f"No significant improvement, max depth reached, or CH score exceeded for cluster {cluster_id}.")
                cluster_id_tensor = torch.tensor([cluster_id], dtype=torch.long).repeat(cluster_data.shape[0])
                sub_clusters.append((cluster_id, cluster_data, cluster_id_tensor, sub_somm.som_layer.prototypes.detach().cpu().numpy(), ch_score))
    ...
    #     X = X.to(self.somm.device)  # Ensure the tensor is on the correct device (CPU or GPU)
    #     n_samples = X.shape[0]

    #     labels = [()] * n_samples  # List of tuples to store the lineage
    #     depths = np.zeros(n_samples, dtype=int)
    #     data_indices = np.arange(n_samples)
    #     parent_cluster_ids = [()] * n_samples  # Initially, all empty tuples

    #     current_depth = 0

    #     while True:
    #         if current_depth not in self.prototypes:
    #             break

    #         # Build a mapping from parent_cluster_id to list of indices
    #         parent_id_to_indices = {}

    #         for idx in data_indices:
    #             parent_id = parent_cluster_ids[idx]
    #             if parent_id not in parent_id_to_indices:
    #                 parent_id_to_indices[parent_id] = []
    #             parent_id_to_indices[parent_id].append(idx)

    #         # For each parent_cluster_id
    #         any_further_splitting = False

    #         for parent_id, indices in parent_id_to_indices.items():
    #             unique_id = (current_depth,) + parent_id if parent_id else (current_depth,)

    #             if unique_id in self.prototypes[current_depth]:
    #                 any_further_splitting = True

    #                 # Convert prototypes and covariance_inv to PyTorch tensors
    #                 prototypes = torch.tensor(self.prototypes[current_depth][unique_id], dtype=torch.float32).to(self.somm.device)
    #                 covariance_inv = torch.tensor(self.covariances[current_depth][unique_id], dtype=torch.float32).to(self.somm.device)
    #                 X_batch = X[indices]

    #                 # Compute distances using PyTorch operations
    #                 diff = X_batch.unsqueeze(1) - prototypes.unsqueeze(0)  # Shape: (batch_size, n_prototypes, input_dim)
    #                 mahalanobis_term = torch.matmul(diff, covariance_inv)  # Shape: (batch_size, n_prototypes, input_dim)
    #                 distances = torch.sum(mahalanobis_term * diff, dim=2)  # Shape: (batch_size, n_prototypes)
    #                 bmu = torch.argmin(distances, dim=1)  # Shape: (batch_size,)

    #                 # Update labels and parent_cluster_ids
    #                 for idx_in_batch, sample_idx in enumerate(indices):
    #                     labels[sample_idx] = parent_cluster_ids[sample_idx] + (bmu[idx_in_batch].item(),)
    #                     parent_cluster_ids[sample_idx] = labels[sample_idx]
    #                     depths[sample_idx] = current_depth + 1
    #             else:
    #                 continue

    #         if not any_further_splitting:
    #             break
    ...
    #     # Initialize a dictionary to store quantization errors
    #     cluster_quantization_errors = {}
    ...
    #                 # Compute quantization error for each subcluster (prototype)
    #                 for proto_idx in range(prototypes.shape[0]):
    #                     # Get the indices of data points assigned to this prototype
    #                     assigned_mask = (bmu == proto_idx)
    #                     if assigned_mask.sum().item() > 0:
    #                         assigned_indices = assigned_mask.nonzero(as_tuple=False).squeeze()
    #                         # Get distances of assigned data points to all prototypes
    #                         assigned_distances = distances[assigned_mask]  # Shape: (n_assigned, n_prototypes)
    #                         # Compute quantization error using the provided function
    #                         qe = torch.mean(torch.min(assigned_distances, dim=1)[0])#quantization_error(assigned_distances)
    #                         # Build the full cluster label
    #                         cluster_label = parent_id + (proto_idx,)
    #                         cluster_quantization_errors[cluster_label] = qe.item()
    #             else:
    #                 continue
    ...
    #     return labels, cluster_quantization_errors
    ...
    def predict(self, X):
        ...
        # Initialize dictionaries to store quantization errors and prototypes
        cluster_quantization_errors = {}
        cluster_prototypes = {}
            ...
            for parent_id, indices in parent_id_to_indices.items():
                ...
                if unique_id in self.prototypes[current_depth]:
                    ...
                    prototypes = torch.tensor(
                        self.prototypes[current_depth][unique_id], dtype=torch.float32
                    ).to(self.somm.device)
                    covariance_inv = torch.tensor(
                        self.covariances[current_depth][unique_id], dtype=torch.float32
                    ).to(self.somm.device)
                    X_batch = X[indices]

                    # Compute distances using PyTorch operations
                    diff = X_batch.unsqueeze(1) - prototypes.unsqueeze(0)  # (batch_size, n_prototypes, input_dim)
                    mahalanobis_term = torch.matmul(diff, covariance_inv)  # (batch_size, n_prototypes, input_dim)
                    distances = torch.sum(mahalanobis_term * diff, dim=2)  # (batch_size, n_prototypes)
                    distances = torch.sqrt(distances)
                    bmu = torch.argmin(distances, dim=1)  # (batch_size,)
                    ...
                    # Compute quantization error and store prototypes for each subcluster (prototype)
                    for proto_idx in range(prototypes.shape[0]):
                        # Get the indices of data points assigned to this prototype
                        assigned_mask = (bmu == proto_idx)
                        if assigned_mask.sum().item() > 0:
                            assigned_indices = assigned_mask.nonzero(as_tuple=False).squeeze()
                            # Get distances of assigned data points to all prototypes
                            assigned_distances = distances[assigned_mask]  # (n_assigned, n_prototypes)
                            # Compute quantization error
                            qe = torch.mean(torch.min(assigned_distances, dim=1)[0])
                            # Build the full cluster label
                            cluster_label = parent_id + (proto_idx,)
                            cluster_quantization_errors[cluster_label] = qe.item()
                            # Store the prototype associated with the cluster_label
                            cluster_prototypes[cluster_label] = prototypes[proto_idx].detach().cpu().numpy()
                else:
                    continue
        ...
        return labels, cluster_quantization_errors, cluster_prototypes
