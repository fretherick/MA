...
# The paper HMM
from typing import Tuple   # add this near the imports
...
#     k = hmm_params["k"]
#     initial_state_prob = np.array(hmm_params["initial_state_prob"])
#     transition_matrix = np.array(hmm_params["transition_matrix"])
#     means = np.array(hmm_params["means"]).reshape(-1, 1)
#     variances = np.array(hmm_params["variances"]).reshape(-1, 1)
...
def main():
    ...
    for i, window in enumerate(windows):
        features = extract_hmm_features_vectorized(X=np_log_rets, l=window)
        all_features[:, :, i*OUT_FEAT:(i+1)*OUT_FEAT] = features[:, max_window:len_path - max_window]
    np_log_rets = np_log_rets[:, max_window:len_path - max_window]
    for i in range(nb_paths):
        all_features[i] = StandardScaler().fit_transform(all_features[i])
    # There are3 duplicated features 
