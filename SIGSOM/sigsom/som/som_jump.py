...
class SOMLayer(nn.Module):
    ...
    def __init__(self, map_size, input_dims, data, prototypes=None):
        ...
        if prototypes is not None:
            ...
            # self.prototypes = nn.Parameter(self.initialize_reference_vectors(data, map_size[0], map_size[1]))
            # print("im initializing")
            # nn.init.xavier_uniform_(self.prototypes.data)
    
    def forward(self, inputs):
        """
        Forward pass calculates the squared Euclidean distance between each input vector and each prototype
        """
        # Shape: (batch_size, 1, input_dim) - Expand inputs for broadcasting
        input_expanded = inputs.unsqueeze(1)
        ...
        diff = input_expanded - prototypes_expanded
        
        # Shape: (batch_size, n_prototypes) - Calculate squared Euclidean distances
        distances = diff.pow(2).sum(dim=2)
        # distances = diff.abs().sum(dim=2)
        return distances
    ...
    def sobol_initialization(self, data):
        ...
        sobol_points_scaled = sobol_points
    ...
    def initialize_reference_vectors(self, data, P, Q):
        """
        Initialize reference vectors for a P x Q SOM grid.
        :param data: A PyTorch tensor of shape (n_samples, n_features).
        :param P: Number of nodes in one dimension of the SOM grid.
        :param Q: Number of nodes in the other dimension of the SOM grid.
        :return: A tensor of shape (P * Q, n_features) containing the initialized reference vectors.
        """
        # Convert data to numpy array
        data_np = data.cpu().numpy()

        # Normalize data
        data_normalized = (data_np - np.mean(data_np, axis=0)) / np.std(data_np, axis=0)

        # Step 1: Calculate the mean vector X̄
        X_mean = np.mean(data_normalized, axis=0)

        # Step 2: Calculate the matrix XtX
        XtX = np.dot(data_normalized.T, data_normalized)

        # Step 3: Calculate the eigenvalues and eigenvectors of XtX
        ...
        # Step 5: Use more than just two principal components
        z1, z2, z3 = eigenvectors[:, :3].T  # Use the first three components instead of just two

        # Step 6: Initialize the reference vectors using the given formula
        reference_vectors = []
        for m in range(P * Q):
            A1_m = -2 + ((m % P) * 4 / (P - 1))
            A2_m = -2 + ((m // P) * 4 / (P - 1))# for a row topology use P instead of Q here
            A3_m = np.random.uniform(-2, 2)  # Adding a third random component
            mu_init_m = (
                X_mean +
                A1_m * np.sqrt(eigenvalues[0]) * z1 +
                A2_m * np.sqrt(eigenvalues[1]) * z2 +
                A3_m * np.sqrt(eigenvalues[2]) * z3  # Third component
            )
            reference_vectors.append(mu_init_m)

        # Convert to tensor
        reference_vectors_tensor = torch.tensor(reference_vectors, dtype=torch.float32)

        # Post scale the initialized means
        post_scaled_means = self.post_scale_initial_means(reference_vectors_tensor, data)
        
        return post_scaled_means

    def post_scale_initial_means(self, initial_means, data):
        """
        Post scale the initialized means to better fit the data.
        :param initial_means: A tensor of shape (P * Q, n_features) containing the initialized reference vectors.
        :param data: The original data tensor.
        :return: A tensor of shape (P * Q, n_features) containing the scaled reference vectors.
        """
        # Calculate data mean and standard deviation
        data_mean = torch.mean(data, dim=0)
        data_std = torch.std(data, dim=0)
        
        # Calculate initial means mean and std
        initial_mean = torch.mean(initial_means, dim=0)
        initial_std = torch.std(initial_means, dim=0)

        # Scale initial means to have the same mean and std as the data
        scaled_means = (initial_means - initial_mean) / initial_std * data_std + data_mean
    ...
    def fit(self, X_train, 
        ...
        for epoch in range(iterations):
            ...
            if (epoch + 1) % jump_penalty_epochs == 0:
                self.logger.info(f"Applying BMU smoothing at epoch {epoch + 1}")
                smoothed_bmu = self.refine_bmu_sequence(bmu, X_train, lambda_jump=lambda_jump)

                # Log new BMU sequence without modifying SOM weights
                self.logger.info(f"Smoothed BMU sequence computed.")
        
        print("Training with jump penalty completed.")

    def predict(self, X, lambda_jump=0.1):
        """
        Predicts the Best Matching Unit (BMU) for input data using the trained SOM,
        incorporating the jump penalty via dynamic programming.
        ...
        with torch.no_grad():
            som_output = self.forward(X)  # Compute distances to SOM prototypes
            bmu_initial = som_output.min(dim=1)[1]  # Initial BMU assignments without smoothing

        # Apply dynamic programming to refine the BMU sequence
        smoothed_bmu = self.refine_bmu_sequence(bmu_initial, X, lambda_jump=lambda_jump)  # Adjust lambda_jump as needed
...
"""FIx arrival cost to;
$arrival_cost = torch.min(
    arrival_cost.view(1, -1) + transition_cost.view(-1, 1) + quantization_loss.view(-1, 1), dim=1
)[0]"""
########################################################################################################################